...
export class Migration20220329092231 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "tag" ("id" varchar(255) not null, "created_at" timestamptz(0) not null, "updated_at" timestamptz(0) not null, "name" varchar(255) not null);',
    );
    this.addSql(
    ...
    this.addSql(
      'alter table "card" add constraint "card_tag_id_foreign" foreign key ("tag_id") references "tag" ("id") on update cascade on delete set null;',
    );
  }
